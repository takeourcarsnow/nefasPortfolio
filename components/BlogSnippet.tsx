...
export const BlogSnippet: React.FC<BlogSnippetProps> = ({ post, index, isOpen, onToggle }) => {
  const contentArr = post.content.en;

  return (
    <div key={post.id ?? post.title ?? index} className="blog-snippet" style={{ marginBottom: 24, textAlign: 'center' }}>
      ...
      {isOpen && (
        <div className="blog-full-content expanded" style={{ textAlign: 'center' }}>
          <div className="blog-content-text">
            {contentArr.map((para, i) => {
              ...
              return <p key={k} style={{ textAlign: 'center' }}>{para}</p>;
            })}
          </div>
        </div>
      )}
